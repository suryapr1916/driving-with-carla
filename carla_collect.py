...
def process_img(image, training_data, vehicle, display, camera_input = 'left'):
...
left_file_name_index  = 'DC_Data/left/' + str(name_of_colecting_sesion) + '_' + str(file_index) + '.npy'
right_file_name_index  = 'DC_Data/right/' + str(name_of_colecting_sesion) + '_' + str(file_index) + '.npy'
...
if os.path.isfile(left_file_name_index) or os.path.isfile(right_file_name_index):
    print('File exists, we will overwrite a file!')
else:
...
try:
    ...
    left_cam_spawn_point = carla.Transform(carla.Location(x=1.5, z=2.4, y =-0.3))
    right_cam_spawn_point = carla.Transform(carla.Location(x=1.5, z=2.4, y =0.3))
    
    # spawn the sensor and attach to vehicle.
    left_cam_sensor = world.spawn_actor(blueprint, left_cam_spawn_point, attach_to=our_vehicle)
    right_cam_sensor = world.spawn_actor(blueprint, right_cam_spawn_point, attach_to=our_vehicle)
    actor_list.append(left_cam_sensor)
    actor_list.append(right_cam_sensor)

    left_cam_sensor.listen(lambda data: process_img(data, left_training_data, our_vehicle, display, camera_input = 'left'))
    right_cam_sensor.listen(lambda data: process_img(data, right_training_data, our_vehicle, display, camera_input = 'right'))
    ...
    while not done:
        ...
        if len(left_training_data) == 256:
            print('saving data...')

            left_temp = left_training_data
            left_training_data = []

            right_temp = right_training_data
            right_training_data = []

            np.save(left_file_name_index, left_temp)
            print(str(left_file_name_index) + 'is saved!')

            np.save(right_file_name_index, right_temp)
            print(str(right_file_name_index) + 'is saved!')
    ...
    left_cam_sensor.stop()
    right_cam_sensor.stop()
    pygame.quit()

    print('Saving data...')
    np.save(left_file_name_index, left_training_data)
    np.save(right_file_name_index, right_training_data)
    print(str(left_file_name_index) + 'is saved!')
    print(str(right_file_name_index) + 'is saved!')
